...
class Decoder(nn.Module):
    def __init__(self, z_dim, hidden_dim):
        ...
        self.fc21 = nn.Linear(hidden_dim, 19793)
        # setup the non-linearities
        self.softplus = nn.Softplus()
...
class VAE(nn.Module):
    ...
    def __init__(self, n_feat=1, z_dim=50, hidden_dim=400, use_cuda=False):
        super(VAE, self).__init__()
        # create the encoder and decoder networks
        self.encoder = Encoder(n_feat, z_dim, hidden_dim)
...
def main(args):
    ...
    train_loader, test_loader, nFeat = get_data()

    # setup the VAE
    vae = VAE(n_feat=nFeat, use_cuda=args.cuda)
...
def get_data():
    dataset = 'CoraFull'
    path = '../data/geometric/CoraFull'
    trainset = CoraFull(path)
    testset = CoraFull(path)
    # print(trainset[0].)
    ...
    return trainLoader, testloader, trainset.num_features
